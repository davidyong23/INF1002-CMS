 ...
 *  - File-backed "StudentRecords" table with columns: ID(int), Name(string), Programme(string), Mark(float)
 ...
 *  - Summary: SHOW SUMMARY  (count, average, highest/lowest with names)
 *  - Unique feature: UNDO (revert the most recent INSERT/UPDATE/DELETE)
 *  - Extras: HELP, EXIT
 *
 * Usage:
 *   Compile:  gcc -std=c99 -O2 cms.c -o cms
 *   Run:      ./cms
 *
 * Database filename convention:
 *   Use: OPEN <TeamName>
 *   This opens/creates "<TeamName>-CMS.txt" in a robust pipe-separated format.
 *
 * File format (pipe-separated to handle spaces): ID|Name|Programme|Mark
 *   Example line: 2301234|Joshua Chen|Software Engineering|70.5
 *
 * Notes:
 *   - The parser accepts flexible commands like:
 ...
 *       EXIT
 *
 * Start-up: prints the required academic declaration block.
 */
...
typedef struct {
    OpType type;
    Student before; // for UPDATE/DELETE
    Student after;  // for INSERT/UPDATE
    int had_before; // flag whether 'before' existed
} UndoEntry;
...
static int parse_between(const char* line, const char* key, char* out, size_t outsz) {
    // find key (case-insensitive) pattern like key=
    const char* p = line;
    size_t klen = strlen(key);
    while (*p) {
        if (strncasecmp(p, key, klen) == 0 && p[klen] == '=') {
            p += klen + 1;
            // Allow quoted values "..." or unquoted up to next key XXX= or end
            if (*p == '\"') {
                p++;
                ...
                // copy until we hit ' KEY=' of another known key or end
                // known keys: ID, NAME, PROGRAMME, MARK
                const char* q = p;
                while (*q) {
                    if (strncasecmp(q, " ID=", 4) == 0 ||
                        strncasecmp(q, " NAME=", 6) == 0 ||
                        strncasecmp(q, " PROGRAMME=", 11) == 0 ||
                        strncasecmp(q, " MARK=", 6) == 0) break;
                    q++;
                }
...
static void cmd_show_all(const char* args) {
    // copy current records to temp for optional sorting
    ...
    // parse sort options
    int sort_by = 0; // 0 none, 1 id, 2 mark
    int desc = 0;
    if (args && *args) {
    ...
    for (int i = 0; i < n_records; ++i) {
        printf("%d\t%s\t%s\t%.1f\n", tmp[i].id, tmp[i].name, tmp[i].programme, tmp[i].mark);
    }
    free(tmp);
}
...
static void cmd_show_summary(void) {
    ...
    for (int i = 0; i < n_records; ++i) {
        sum += records[i].mark;
        if (records[i].mark > records[hi_idx].mark) hi_idx = i;
        if (records[i].mark < records[lo_idx].mark) lo_idx = i;
    }
...
static void cmd_query(const char* line) {
    char buf[64];
    if (!parse_between(line, "ID", buf, sizeof(buf))) {
        printf("CMS: Please provide ID. e.g., QUERY ID=2401234\n");
        return;
    }
    int id = atoi(buf);
...
static void cmd_insert(const char* line) {
    ...
    if (!parse_between(line, "ID", s_id, sizeof(s_id))) {
        printf("CMS: Please provide ID. e.g., INSERT ID=2401234 Name=\"Michelle Lee\" Programme=\"Information Security\" Mark=73.2\n");
        return;
    }
    int id = atoi(s_id);
    ...
    if (!parse_between(line, "NAME", s_name, sizeof(s_name)) ||
        ...
        !parse_between(line, "MARK", s_mark, sizeof(s_mark))) {
        printf("CMS: Missing fields. Required: NAME, PROGRAMME, MARK.\n");
        return;
    }
    if (n_records >= MAX_RECORDS) {
        printf("CMS: Cannot insert, reached max records.\n");
        return;
    }
    Student s;
    s.id = id;
...
static void cmd_update(const char* line) {
    ...
    if (!parse_between(line, "ID", s_id, sizeof(s_id))) {
        printf("CMS: Please provide ID. e.g., UPDATE ID=2401234 Mark=69.8\n");
        return;
    }
    int id = atoi(s_id);
    int idx = find_index_by_id(id);
    ...
    if (!has_name && !has_prog && !has_mark) {
        printf("CMS: Nothing to update. Provide NAME/PROGRAMME/MARK.\n");
        return;
    }
    ...
    if (has_mark) { records[idx].mark = (float)atof(s_mark); }
...
static void cmd_delete(const char* line) {
    char s_id[64];
    if (!parse_between(line, "ID", s_id, sizeof(s_id))) {
        printf("CMS: Please provide ID. e.g., DELETE ID=2301234\n");
        return;
    }
    int id = atoi(s_id);
    int idx = find_index_by_id(id);
...
static void cmd_undo(void) {
    ...
    if (u.type == OP_INSERT) {
        // undo insert => delete that ID
        int idx = find_index_by_id(u.after.id);
        if (idx >= 0) {
            for (int i = idx + 1; i < n_records; ++i) records[i-1] = records[i];
            n_records--;
            printf("CMS: UNDO successful (reverted last INSERT of ID=%d).\n", u.after.id);
        } else {
        ...
        if (idx >= 0) {
            records[idx] = u.before;
            printf("CMS: UNDO successful (reverted last UPDATE of ID=%d).\n", u.before.id);
        } else {
            printf("CMS: UNDO failed (record not found).\n");
        }
        ...
        // re-insert 'before'
        if (n_records >= MAX_RECORDS) {
            printf("CMS: UNDO failed (max records reached).\n");
            return;
        }
...
static void cmd_help(void) {
    ...
    printf("  QUERY  ID=<int>\n");
    printf("  UPDATE ID=<int> [Name=\"<str>\"] [Programme=\"<str>\"] [Mark=<float>]\n");
...
static void ensure_filename_from_team(void) {
    if (team_name[0]) {
        snprintf(db_filename, sizeof(db_filename), "%s-CMS.txt", team_name);
    }
}
...
int main(void) {
    ...
    while (1) {
        ...
        // Make a copy for parsing arguments
        char cmd[MAX_LINE]; strncpy(cmd, line, sizeof(cmd)-1); cmd[sizeof(cmd)-1]=0;

        // Uppercase a copy to identify the verb
        char up[MAX_LINE]; strncpy(up, line, sizeof(up)-1); up[sizeof(up)-1]=0;
        for (char* c = up; *c; ++c) *c = (char)toupper((unsigned char)*c);
        ...
        if (strncmp(up, "EXIT", 4) == 0 || strncmp(up, "QUIT", 4) == 0) {
            ...
            // Format: OPEN <TeamName>
            char* p = cmd + 4;
            while (*p && isspace((unsigned char)*p)) p++;
            if (!*p) {
                printf("CMS: Please provide a team name. e.g., OPEN P1_1\n");
                continue;
            }
            strncpy(team_name, p, sizeof(team_name)-1); team_name[sizeof(team_name)-1]=0;
            trim(team_name);
            ensure_filename_from_team();
            if (load_db(db_filename)) {
                printf("CMS: The database file \"%s\" is successfully opened.\n", db_filename);
            } else {
                // create empty file on first SAVE
                n_records = 0;
                printf("CMS: New database will be created: \"%s\" (no existing file found yet).\n", db_filename);
            }
            ...
            if (!db_filename[0]) {
                printf("CMS: Please OPEN <TeamName> first to determine the database filename.\n");
            } else {
                if (save_db(db_filename)) {
                    printf("CMS: The database file \"%s\" is successfully saved.\n", db_filename);
                } else {
                    printf("CMS: Failed to save database file.\n");
                }
            }
